...
export default function Home() {
  ...
  const getChatsRepeatedly = () => {
    if (recieverData && localStorage.getItem("chatRoomID")) {
      setTimeout(() => {
        getMessages();
        getChatsRepeatedly();
      }, 5000);
  ...
  const getUserChat = () => {
    onSnapshot(
      query(collection(db, "chatroom"), where("sendBy", "==", "TawaGrill")),
      (snapshot) => {
        let userChatList = snapshot.docs.map((doc) => {
          return doc.data();
        });
  ...
  useEffect(() => {
    ...
    if (recieverData) {
      const unsub = onSnapshot(
        query(
          collection(db, "messages"),
          where("chatRoomID", "==", recieverData.chatRoomID)
        ),
        (snapshot) => {
          ...
          if (!localStorage.getItem("restaurantID")) {
            localStorage.setItem("sentTo", messageList[0].sendBy);
          }
    ...
    try {
      if (recieverData) {
        await addDoc(collection(db, "messages"), {
          ...
          sendBy: localStorage.getItem("restaurantName"),
